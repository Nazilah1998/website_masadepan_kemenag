      ...
      <div className="relative w-full max-w-md">
        <div className="rounded-4xl border border-slate-200 bg-white p-6 shadow-2xl sm:p-8">
          ...
          <form onSubmit={f.handleSubmit} className="mt-8 space-y-5">
            ...
            {f.success && <StatusAlert type="success" message={f.success} />}
            ...
            >
              {f.submitting ? "Mengirim..." : "Kirim tautan reset"}
            </button>
          </form>

          <div className="mt-6 rounded-2xl bg-slate-50 px-4 py-3 text-xs leading-6 text-slate-500">
            Pastikan URL <span className="font-semibold">/update-password</span> sudah masuk ke daftar redirect URL di Supabase Authentication.
          </div>
        </div>
      </div>
    </section>
...
function ForgotPasswordHeader() {
  return (
    <>
      ...
      <div className="mt-8">
        <div className="inline-flex rounded-full bg-emerald-50 px-3 py-1 text-xs font-semibold uppercase tracking-[0.24em] text-emerald-700">Reset Password</div>
        <h1 className="mt-4 text-3xl font-bold tracking-tight text-slate-950">Lupa password admin</h1>
        <p className="mt-3 text-sm leading-7 text-slate-600">Masukkan email admin Anda. Sistem akan mengirim tautan reset password ke email tersebut.</p>
      </div>
    </>
  );
}
